...
```
docker run -v /path/host/mount/:/path/container/mount/ operams config.file
```
To process data, a config file containing base directory in the host machine has to be mounted into the container. The data in- and output directory should be subdirectories of the base directory. An example is shown below for running the test dataset. Here, it is assumed that the repo is cloned to /home/myuser/git/OPERA-MS/).

```
docker run -v /home/myuser/git/OPERA-MS/:/home/testuser/OPERA-MS/ operams sample_config.config
```
